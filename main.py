...
def start_timer():
    ...
    reps += 1
    if reps % 8 == 0:
        my_label.config(text="Break", font=(FONT_NAME, 35, "bold"), bg=YELLOW, fg=RED)
        count_down(long_break_sec)
    elif reps % 2 == 0:
        my_label.config(text="Break", font=(FONT_NAME, 35, "bold"), bg=YELLOW, fg=PINK)
        count_down(short_break_sec)
    else:
        my_label.config(text="Work", font=(FONT_NAME, 35, "bold"), bg=YELLOW, fg=GREEN)
        count_down(work_sec)
